...
class Camera
  ...
  public Camera(
    ...
    // [Dart Rivals patch] Apply MediaSettings.fps to the WHOLE session
    // (preview + image stream), mirroring camera_avfoundation which sets
    // activeVideoMin/MaxFrameDuration at configuration time. Upstream only
    // applies fps while a video recording is active.
    applyMediaSettingsFpsCap();
  ...
  /**
   * [Dart Rivals patch] Caps the session AE target FPS range to
   * MediaSettings.fps when provided, picking the closest SUPPORTED range so
   * the CaptureRequest never gets an illegal value: prefer the lowest upper
   * bound >= fps, tie-broken toward a fixed (narrow) range; fall back to the
   * plugin default when no such range exists.
   */
  private void applyMediaSettingsFpsCap() {
    if (videoCaptureSettings.fps == null || videoCaptureSettings.fps.intValue() <= 0) {
      return;
    }
    final int targetFps = videoCaptureSettings.fps.intValue();
    ...
    if (best != null) {
      ...
      fpsRange.setValue(best);
      cameraFeatures.setFpsRange(fpsRange);
      Log.i(TAG, "applyMediaSettingsFpsCap: requested " + targetFps + " -> using " + best);
    }
  }
  ...
  public String stopVideoRecording() {
    ...
    // [Dart Rivals patch] Re-apply the session-wide fps cap the reset above
    // just discarded.
    applyMediaSettingsFpsCap();
